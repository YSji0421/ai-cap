...
import { faqApi, sessionApi } from '../services/api';
...
export default function ConsultationRoomPage() {
  ...
  // Load recommendation context if coming from recommendation page
  useEffect(() => {
    const ctx = localStorage.getItem('recommendationContext');
    if (ctx) {
      try {
        setRecommendationContext(JSON.parse(ctx));
      } catch (e) { /* ignore */ }
    }
  }, []);
  ...
  const insuranceType = localStorage.getItem('selectedInsuranceType') || 'TRAFFIC';
  const INSURANCE_LABELS = { TRAFFIC: '교통사고', INJURY: '상해', FIRE: '화재', LIFE: '생명' };

  // FAQ DB search first, then fallback to AI
  const handleAnalyze = async () => {
    ...
    // Step 1: Search FAQ DB for instant cached answers
    try {
      const faqResponse = await faqApi.search(text, insuranceType);
      const faqData = faqResponse.data;
      if (faqData.hasResults) {
        setFaqResults(faqData.faqResults);
        setClauseResults(faqData.clauseResults || []);
        setTranscript(prev => [...prev, {
          speaker: '📚 FAQ',
          text: `관련 FAQ ${faqData.faqResults.length}건 발견 - 즉시 표시됨`,
          time: new Date().toLocaleTimeString('ko-KR'),
          isAi: true
        }]);
      }
    } catch (e) {
      console.log('FAQ search failed, proceeding with AI:', e.message);
    }

    // Step 2: Call Gemini AI for deeper analysis
    try {
      let contextText = text;
      if (recommendationContext) {
        contextText += `\n\n[추천 상품 컨텍스트] 고객 고민: ${recommendationContext.concerns}, 추천 상품: ${recommendationContext.productName}`;
      }
      const result = await analyzeConsultation(contextText, insuranceType);
  ...
  const startRecording = () => {
    const streams = [];
    if (localVideoRef.current?.srcObject) streams.push(localVideoRef.current.srcObject);
    if (remoteVideoRef.current?.srcObject) streams.push(remoteVideoRef.current.srcObject);
    if (streams.length === 0) return;

    const stream = streams[0]; // Record local stream
  ...
  const downloadRecording = () => {
    ...
    a.href = url;
    a.download = `consultation-${roomId}-${new Date().toISOString().slice(0, 10)}.webm`;
    a.click();
  ...
  const endCall = async () => {
    ...
    const sessionData = {
      ...
      title: `AI 상담 - ${INSURANCE_LABELS[insuranceType] || '교통사고'} - ${new Date().toLocaleDateString('ko-KR')}`,
      transcript,
      analysis: aiAnalysis,
      duration: elapsed,
      insuranceType,
    ...
    try {
      ...
      await sessionApi.saveSession(roomId, {
        roomId,
    ...
    // Clean up recommendation context
    localStorage.removeItem('recommendationContext');
    ...
    <div className="room-wrap">
          ...
          <div className="pdf-scroll">
            ...
            {recommendationContext && (
              <div className="recommendation-context">
                <h4>📋 추천 상담 정보</h4>
                <p><strong>추천 상품:</strong> {recommendationContext.productName}</p>
                <p><strong>고객 고민:</strong> {recommendationContext.concerns}</p>
              </div>
            )}
            ...
            {faqResults && faqResults.length > 0 && (
              <div className="faq-results">
                <h4>📚 관련 FAQ ({faqResults.length}건)</h4>
                {faqResults.map((faq, i) => (
                  <div key={i} className="faq-card">
                    <div className="faq-question">Q. {faq.question}</div>
                    <div className="faq-answer">{faq.answer}</div>
                    {faq.legalReference && (
                      <div className="faq-legal">📎 {faq.legalReference}</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            {clauseResults.length > 0 && (
              <div className="clause-results">
                <h4>⚖️ 관련 법률조항</h4>
                {clauseResults.map((clause, i) => (
                  <div key={i} className="clause-card">
                    <div className="clause-number">{clause.clauseNumber}</div>
                    <div className="clause-title-text">{clause.clauseTitle}</div>
                    <div className="clause-content">{clause.clauseContent}</div>
                  </div>
                ))}
              </div>
            )}

            {/* Default policy doc when no FAQ results */}
            {(!faqResults || faqResults.length === 0) && (
      ...
      <main className="panel-video">
        <div className="video-topbar">
          ...
          <span className="insurance-badge">{INSURANCE_LABELS[insuranceType] || '교통사고'}</span>
          ...
          {isRecording && <span className="recording-badge">🔴 녹화 중</span>}
        </div>
        ...
        <div className="ctrl-bar">
          ...
          <button
            ...
            title="녹화"
          >
            {isRecording ? '⏹️ 녹화 중지' : '⏺️ 녹화'}
          </button>
          {recordingBlob && (
            <button className="ctrl-btn" onClick={downloadRecording} title="녹화 다운로드">
              💾 저장
            </button>
          )}
      ...
      <aside className="panel-ai">
        ...
        {aiAnalysis && (
          <div className="analysis-box">
            <div className="section-title">✦ AI 분석 결과</div>
